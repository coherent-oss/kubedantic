...
class PodReplacementPolicy(Enum):
    Failed = "Failed"
...
class PodFailurePolicyRule(BaseModel):
    action: Action = Field(
        ...
        description=(
            ...
            " is an action which might be taken on a pod failure - the pod failure is"
            " handled in the default way - the counter towards .backoffLimit,"
            " represented by the job's .status.failed field, is incremented.\n -"
            ' `"FailIndex"` This is an action which might be taken on a pod failure -'
            " mark the Job's index as failed to avoid restarts within this index. This"
            " action can only be used when backoffLimitPerIndex is set. This value is"
            ' beta-level.\n - `"FailJob"` This is an action which might be taken on a'
            " pod failure - mark the pod's job as Failed and terminate all running"
            ' pods.\n - `"Ignore"` This is an action which might be taken on a pod'
            " failure - the counter towards .backoffLimit, represented by the job's"
            " .status.failed field, is not incremented and a replacement pod is"
            " created."
        ),
    )
...
class JobSpec(BaseModel):
    ...
    podReplacementPolicy: Optional[PodReplacementPolicy] = Field(
        default=None,
        description=(
            ...
            " enable the JobPodReplacementPolicy feature toggle. This is on by"
            ' default.\n\nPossible enum values:\n - `"Failed"` means to wait until a'
            " previously created Pod is fully terminated (has phase Failed or"
            ' Succeeded) before creating a replacement Pod.\n - `"TerminatingOrFailed"`'
            " means that we recreate pods when they are terminating (has a"
