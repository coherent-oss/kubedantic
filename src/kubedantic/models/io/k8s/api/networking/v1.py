...
class IngressBackend(BaseModel):
    resource: Optional[v1_1.TypedLocalObjectReference] = Field(
        default=None,
        description=(
...
class NetworkPolicySpec(BaseModel):
    ...
    policyTypes: Optional[List[PolicyType]] = Field(
        default=None,
        description=(
...
class HTTPIngressPath(BaseModel):
    ...
    pathType: PathType = Field(
        ...
        description=(
            ...
            ' identically to Prefix or Exact path types.\n - `"Prefix"` matches based'
            " on a URL path prefix split by '/'. Matching is case sensitive and done on"
            " a path element by element basis. A path element refers to the list of"
            " labels in the path split by the '/' separator. A request is a match for"
            " path p if every p is an element-wise prefix of p of the request path."
            " Note that if the last element of the path is a substring of the last"
            " element in request path, it is not a match (e.g. /foo/bar matches"
            " /foo/bar/baz, but does not match /foo/barbaz). If multiple matching paths"
            " exist in an Ingress spec, the longest matching path is given priority."
